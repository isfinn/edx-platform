...
class ExperimentWaffleFlag(CourseWaffleFlag):
    ...
    If a user is not forced into a specific bucket, then they will be randomly
    assigned a default bucket based on hash function of their username, the name of the
    experiment, and the course_key with which `get_bucket` or `is_enabled` is called.
    If you wish for any given user to be assigned the *same* default bucket across all
    courses, then include `use_course_aware_bucketing=False` when initializing this class.
    ...
    This is only applicable when `use_course_aware_bucketing` is True.
    ...
    def get_bucket(self, course_key, track=True):
        """
        ...
        # If experiment is using course-aware bucketing, use course key in experiment
        # name to separate caching and segment calls per-course-run.
        ...
        if self.use_course_aware_bucketing and self.experiment_id is not None:
